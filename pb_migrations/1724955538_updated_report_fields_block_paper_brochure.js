...
migrate((db) => {
  ...
  // remove
  collection.schema.removeField("ap4wzrxu")

  // remove
  collection.schema.removeField("qt6t8w02")

  // remove
  collection.schema.removeField("as7kko6n")

  // remove
  collection.schema.removeField("6r7ur1pn")

  // remove
  collection.schema.removeField("pys9vf3j")

  // remove
  collection.schema.removeField("ftljzmgw")

  // remove
  collection.schema.removeField("l200ejuo")

  // remove
  collection.schema.removeField("tqhn6zgo")
  ...
  collection.options = {
    "query": "SELECT\n  id,\n  order_id,\n  section_id,\n  \"order\",\n  \"name\",\n  \"value\",\n  units,\n  rate,\n  cost\nFROM (\n  -- Бумага на блок\n  SELECT\n    concat(o.id, '_block_paper_outgo') as id,\n    o.id as order_id,\n    '000006_materals' as section_id,\n    100 as \"order\",\n    concat(bp.name, ' ', obp.block_format) as \"name\",\n    obc.block_full_sheets as \"value\",\n    'шт.' as units,\n    obp.block_paper_price as rate,\n    ceiling(obc.block_full_sheets*obp.block_paper_price) as cost\n  FROM orders o,\n        orders_block_processed obp,\n        orders_block_calc obc,\n        papers bp\n  WHERE obp.id = o.id\n  AND   obc.id = o.id\n  AND   bp.id = o.block_paper\n  AND   obp.block_format != '700*500'\n\n  UNION\n\n  -- Бумага на блок 700*500\n  SELECT\n    concat(o.id, '_block_paper_outgo_ofset') as id,\n    o.id as order_id,\n    '000006_materals' as section_id,\n    100 as \"order\",\n    concat(bp.name, ' 700*1000') as \"name\",\n    ceiling(obc.block_full_sheets/2) as \"value\",\n    'шт.' as units,\n    obp.block_paper_price as rate,\n    ceiling(ceiling(obc.block_full_sheets/2)*obp.block_paper_price) as cost\n  FROM orders o,\n        orders_block_processed obp,\n        orders_block_calc obc,\n        papers bp\n  WHERE obp.id = o.id\n  AND   obc.id = o.id\n  AND   bp.id = o.block_paper\n  AND   obp.block_format = '700*500'\n)"
  }
